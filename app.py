...
_CURRENCY_SYMBOLS = {'gbp': '£', 'usd': '$', 'eur': '€', 'aud': 'A$', 'cad': 'C$'}
_ZERO_DECIMAL_CURRENCIES = {'jpy', 'krw', 'vnd', 'clp', 'xof', 'xaf'}


def format_money(minor_units, currency):
    """Format a minor-unit integer amount for display. Never used for maths —
    the amount stays an integer everywhere; this is presentation only."""
    cur = (currency or '').lower()
    symbol = _CURRENCY_SYMBOLS.get(cur, '')
    try:
        minor = int(minor_units)
    ...
    return f'{body} {(currency or "").upper()}'.strip()
...
def ensure_billing_schema(conn):
    """Read-only Stripe Connect billing visibility. All names are Connect-specific
    so they never collide with the platform's own subscription billing columns
    (users.stripe_customer_id / stripe_subscription_id) or /stripe/webhook."""
    global _billing_schema_ready
    if _billing_schema_ready:
        return
    with conn.cursor() as cursor:
        # Agency rows: the connected Stripe account.
        cursor.execute('ALTER TABLE public.users ADD COLUMN IF NOT EXISTS stripe_account_id text')
        cursor.execute('ALTER TABLE public.users ADD COLUMN IF NOT EXISTS stripe_connected_at timestamptz')
        cursor.execute('ALTER TABLE public.users ADD COLUMN IF NOT EXISTS stripe_livemode boolean')
        # Client rows: their customer id *inside the agency's connected account*
        # (distinct from stripe_customer_id, which is our own subscription customer).
        cursor.execute('ALTER TABLE public.users ADD COLUMN IF NOT EXISTS stripe_connect_customer_id text')
        cursor.execute(
            'CREATE UNIQUE INDEX IF NOT EXISTS idx_users_agency_connect_customer '
            'ON public.users (agency_id, stripe_connect_customer_id) '
            'WHERE stripe_connect_customer_id IS NOT NULL'
        )
        cursor.execute(
        ...
        # Local cache of invoice state — every portal + MCP read hits this, never Stripe.
        cursor.execute(
            '''
            CREATE TABLE IF NOT EXISTS public.invoice_cache (
                id                 bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                agency_id          bigint NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_invoice_cache_status ON public.invoice_cache (agency_id, status, due_date)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_invoice_cache_client ON public.invoice_cache (client_id)')
...
def ensure_documents_schema(conn):
    """Per-agency policy documents (T&Cs, privacy policy, ...) that a client must
    read and accept, with a timestamped acceptance audit trail."""
    ...
    with conn.cursor() as cursor:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_agency_documents_agency ON public.agency_documents (agency_id, position)')
        cursor.execute(
            '''
            CREATE TABLE IF NOT EXISTS public.document_acceptances (
                id          bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                document_id bigint NOT NULL REFERENCES public.agency_documents(id) ON DELETE CASCADE,
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_doc_acceptances_client ON public.document_acceptances (client_id)')
    _documents_schema_ready = True


def get_agency_documents(cursor, agency_id):
    cursor.execute(
        'SELECT id, title, body, position FROM public.agency_documents '
        'WHERE agency_id = %s ORDER BY position, id',
        (agency_id,),
    )
    return [{'id': r[0], 'title': r[1], 'body': r[2], 'position': r[3]} for r in cursor.fetchall()]


def unaccepted_documents(cursor, client_id, agency_id):
    """Ordered list of documents this client still needs to accept."""
    cursor.execute(
        '''SELECT d.id, d.title, d.body
           ...
           WHERE d.agency_id = %s
             AND NOT EXISTS (
                 ...
                 WHERE a.document_id = d.id AND a.client_id = %s)
           ORDER BY d.position, d.id''',
        (agency_id, client_id),
    )
    return [{'id': r[0], 'title': r[1], 'body': r[2]} for r in cursor.fetchall()]


# Paths that must stay reachable even for a client who hasn't accepted yet
# (the acceptance flow itself, auth, static, and non-portal endpoints).
_TERMS_EXEMPT_PREFIXES = (
    '/accept-terms', '/logout', '/static', '/sign-in', '/sign-up', '/invite',
    '/mcp', '/oauth', '/stripe', '/brand', '/.well-known', '/sitemap', '/robots',
)


@app.before_request
def _require_document_acceptance():
    """Gate clients who have unaccepted agency documents to the acceptance page.
    Cached in the session so it costs one query per login; re-checks on next
    login (a fresh session has no `terms_ok`), which also catches newly-added
    documents for existing clients."""
    ...
    if any(path.startswith(p) for p in _TERMS_EXEMPT_PREFIXES):
        return
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if not row:
                    return
                role, agency_id = row
                if role != 'client' or not agency_id:
                    session['terms_ok'] = True  # agencies never gate
                    return
                cursor.execute(
                    '''SELECT COUNT(*) FROM public.agency_documents d
                       WHERE d.agency_id = %s AND NOT EXISTS (
                           SELECT 1 FROM public.document_acceptances a
                           WHERE a.document_id = d.id AND a.client_id = %s)''',
                    (agency_id, user_id),
                )
        ...
        return  # never hard-block the portal on a gate error
    if pending == 0:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if is_agency:
                ...
                billing = stripe_connect.get_billing_status(cursor, session.get('user_id'))
                if billing.get('connected'):
                    # Per-client open-invoice summary (Unassigned bucket for
                    # invoices with no mapped client) for the portal UI.
                    cursor.execute(
                        '''SELECT COALESCE(u.name, u.email, 'Unassigned'), ic.currency,
                                  SUM(ic.amount_due), COUNT(*)
                           FROM public.invoice_cache ic
                           LEFT JOIN public.users u ON u.id = ic.client_id
                           WHERE ic.agency_id = %s AND ic.status = 'open'
                           GROUP BY 1, ic.currency
                           ORDER BY 1''',
                        (session.get('user_id'),),
                    )
                    grouped = {}
                    for cname, currency, total, count in cursor.fetchall():
                        entry = grouped.setdefault(cname, {'name': cname, 'count': 0, 'amounts': []})
                        entry['count'] += count
                        entry['amounts'].append(format_money(total, currency))
                    billing_rows = list(grouped.values())

            documents = []
            edit_doc = None
            if is_agency:
                documents = get_agency_documents(cursor, session.get('user_id'))
                edit_id = request.args.get('edit')
                if edit_id and edit_id.isdigit():
                    edit_doc = next((d for d in documents if d['id'] == int(edit_id)), None)
...
# --------------------------------------------------------------------------- #
# Policy documents: agency management + client acceptance
# --------------------------------------------------------------------------- #
@app.route('/settings/documents/save', methods=['POST'])
def documents_save():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('sign_in'))
    ...
    if not title or not body:
        return redirect(url_for('settings', doc='empty'))
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            if not is_agency_role(get_user_role(cursor, user_id)):
                abort(403)
            if doc_id and doc_id.isdigit():
                cursor.execute(
                    'UPDATE public.agency_documents SET title = %s, body = %s, updated_at = now() '
                    'WHERE id = %s AND agency_id = %s',
                    (title, body, int(doc_id), user_id),
                )
            else:
                cursor.execute(
                    'SELECT COALESCE(MAX(position), -1) + 1 FROM public.agency_documents WHERE agency_id = %s',
                    (user_id,),
                )
                pos = cursor.fetchone()[0]
                cursor.execute(
                    'INSERT INTO public.agency_documents (agency_id, title, body, position) VALUES (%s, %s, %s, %s)',
                    (user_id, title, body, pos),
                )
    return redirect(url_for('settings', doc='saved'))


@app.route('/settings/documents/<int:doc_id>/delete', methods=['POST'])
def documents_delete(doc_id):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            if not is_agency_role(get_user_role(cursor, user_id)):
                abort(403)
            cursor.execute('DELETE FROM public.agency_documents WHERE id = %s AND agency_id = %s', (doc_id, user_id))
    return redirect(url_for('settings', doc='deleted'))


@app.route('/settings/documents/<int:doc_id>/move', methods=['POST'])
def documents_move(doc_id):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            if not is_agency_role(get_user_role(cursor, user_id)):
                abort(403)
            docs = get_agency_documents(cursor, user_id)
            ...
            if 0 <= j < len(ids):
                ...
                cursor.execute('UPDATE public.agency_documents SET position = %s WHERE id = %s AND agency_id = %s',
                               (j, ids[i], user_id))
                cursor.execute('UPDATE public.agency_documents SET position = %s WHERE id = %s AND agency_id = %s',
                               (i, ids[j], user_id))
    return redirect(url_for('settings'))
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if not row or row[0] != 'client' or not row[1]:
                session['terms_ok'] = True
                return redirect(url_for('dashboard'))
            pending = unaccepted_documents(cursor, user_id, row[1])
            if not pending:
                session['terms_ok'] = True
                return redirect(url_for('dashboard'))
    return render_template('accept-terms.html', documents=pending)


@app.route('/accept-terms/<int:doc_id>', methods=['POST'])
def accept_terms_record(doc_id):
    user_id = session.get('user_id')
    if not user_id:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            agency_id = row[1]
            # The document must belong to this client's agency.
            cursor.execute('SELECT 1 FROM public.agency_documents WHERE id = %s AND agency_id = %s', (doc_id, agency_id))
            if not cursor.fetchone():
                return jsonify({'error': 'not found'}), 404
            cursor.execute(
                'INSERT INTO public.document_acceptances (document_id, client_id, accepted_at) '
                'VALUES (%s, %s, now()) ON CONFLICT (document_id, client_id) DO NOTHING',
                (doc_id, user_id),
            )
            remaining = len(unaccepted_documents(cursor, user_id, agency_id))
    if remaining == 0:
        session['terms_ok'] = True
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            if row and check_password_hash(row[2], password):
                ...
                session['email'] = row[1]
                session.pop('terms_ok', None)  # re-check document acceptance on every login
                session.modified = True
                return redirect(url_for('dashboard'))
...
from stripe_connect import register_stripe
register_stripe(app, _sys.modules[__name__])
